...
def matrix_mul(m_a, m_b):
    ...
    if all(list(map(lambda x: all(list(map(lambda x: type(x) == int or  type(x) == float, x))), m_a))) is False:
        raise TypeError('m_a should contain only integers or floats')
    if all(list(map(lambda x: all(list(map(lambda x: type(x) == int or  type(x) == float, x))), m_b))) is False:
